...
export default function Dbaside() {
    const router = useRouter();
    const pathname = usePathname();

    const handlerLogout = async () => {
        const c = await fetch('/api/logout', { method: "DELETE", })
            ...
            <div className="w-64 h-screen bg-white border-r-2 max-425:hidden">
                ...
                <div className='w-64 py-3 flex flex-col'>
                    ...
                    {dbnavlist?.map((item, index) => (
                        <Link
                            key={index}
                            style={{ backgroundColor: pathname === item.link ? '#aaa' : '' }}
                            href={item.link} className='h-10 ml-5 px-10 rounded-l-xl text-left content-center hover:bg-slate-200'>{item.name}</Link>
                    ))}
                </div>
